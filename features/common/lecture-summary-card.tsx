...
export function LectureSummaryCard({
  ...
  const content = (
    <Card className="group overflow-hidden border-white/80 bg-[linear-gradient(180deg,#fbfffd_0%,#ffffff_52%,#f6fbff_100%)] transition-all duration-200 hover:-translate-y-1 hover:shadow-[0_24px_46px_-30px_rgba(15,23,42,0.35)]">
      <CardHeader className="space-y-4 pb-4">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 space-y-3">
            <div>
              <div className="flex items-center gap-2">
                <Badge variant="secondary" className="bg-emerald-50 font-semibold text-emerald-700">
              ...
              <CardTitle className="mt-3 text-[20px] leading-7">{lecture.courseName}</CardTitle>
              <p className="mt-1 line-clamp-1 text-sm text-muted-foreground">
                {lecture.professor} · {lecture.section}분반 · {lecture.dayTime}
              </p>
            </div>
            <div className="rounded-[22px] border border-emerald-100 bg-emerald-50/80 px-4 py-3 text-sm text-emerald-900">
              <div className="inline-flex items-center gap-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-emerald-700">
                <Sparkles className="h-3.5 w-3.5" />
                요약
              </div>
              <p className="mt-2 text-sm leading-6 text-emerald-950/85">{reviewSummary}</p>
            </div>
          ...
          <div className="rounded-[26px] border border-emerald-100 bg-[linear-gradient(180deg,#ecfdf5_0%,#ffffff_100%)] px-4 py-4 text-center text-emerald-700 shadow-[0_20px_32px_-24px_rgba(16,185,129,0.65)]">
            <p className="text-[11px] font-semibold uppercase tracking-[0.18em]">평균 점수</p>
            <p className="mt-1 text-2xl font-semibold">{lecture.averageHoneyScore.toFixed(1)}</p>
            <p className="mt-1 text-[11px] text-emerald-700/70">리뷰 {lecture.reviewCount}개 기반</p>
          </div>
        </div>
      </CardHeader>
