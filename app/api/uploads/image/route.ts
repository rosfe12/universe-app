...
export async function POST(request: Request) {
  try {
    ...
    if (error) {
      logServerEvent("error", "image_upload_failed", {
        userId: user.id,
        target,
    ...
    logServerEvent("error", "image_upload_unhandled", {
...
export async function DELETE(request: Request) {
  try {
    ...
    if (error) {
      logServerEvent("error", "image_delete_failed", {
        userId: user.id,
        path,
        message: error.message,
      });
    ...
    logServerEvent("error", "image_delete_unhandled", {
      message: error instanceof Error ? error.message : "unknown",
    });
