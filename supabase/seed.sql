...
with seeded_posts as (
  ...
  from (
    values
      ...
      ('41111111-1111-4111-8111-111111111157'::uuid, '3d666666-6666-4666-8666-666666666666'::uuid, 'community'::public.post_category, null, '[공식] 숭실대 진로취업센터는 재학생·졸업생·지역청년까지 같이 지원하네요', '숭실대 진로취업센터 소개 페이지를 보니 재학생뿐 아니라 졸업생과 지역 청년까지 열린 허브 역할을 한다고 명시돼 있었습니다. 학교 안 취업지원 범위가 넓어서 저학년 때부터 자주 보는 편이 좋아 보여요.', 'bbbbbbb6-bbbb-4bbb-8bbb-bbbbbbbbbbb6'::uuid, 'global'::public.content_scope, 19, 'schoolDepartment'::public.visibility_level, '{"tags":["취업정보","숭실대","진로취업센터"]}'::jsonb),
      ('41111111-1111-4111-8111-111111111158'::uuid, '3f888888-8888-4888-8888-888888888888'::uuid, 'community'::public.post_category, null, '[공식] 홍익 HIVE는 창업교육·멘토링·팀빌딩까지 학교 안에서 이어지네요', '홍익대 공식 학생활동 페이지를 보니 HIVE가 창업 기초 교육, 팀 빌딩, 멘토링, 네트워킹을 꾸준히 운영하고 있었습니다. 취업 탭에서도 창업 쪽 진로를 보는 사람이라면 학교 안 자원을 먼저 확인할 만합니다.', 'ccccccc3-cccc-4ccc-8ccc-ccccccccccc3'::uuid, 'global'::public.content_scope, 18, 'schoolDepartment'::public.visibility_level, '{"tags":["취업정보","홍익대","창업"]}'::jsonb),
      ('41111111-1111-4111-8111-111111111159'::uuid, '3b444444-4444-4444-8444-444444444444'::uuid, 'community'::public.post_category, null, '[공식] 한국외대 대학일자리플러스센터가 고용노동부 평가 우수 3회를 받았다고 공지됐어요', '한국외대 공식 메인 공지에서 대학일자리플러스센터가 고용노동부 연차 성과평가에서 우수 등급을 3회 달성했다고 안내했습니다. 취업지원 체계가 실제로 잘 굴러가는 학교인지 볼 때 이런 공식 지표가 꽤 참고됩니다.', 'bbbbbbb9-bbbb-4bbb-8bbb-bbbbbbbbbbb9'::uuid, 'global'::public.content_scope, 17, 'schoolDepartment'::public.visibility_level, '{"tags":["취업정보","한국외대","대학일자리플러스센터"]}'::jsonb)
...
with seeded_comments as (
  ...
  from (
    values
      ('51111111-1111-4111-8111-111111111201'::uuid, '41111111-1111-4111-8111-111111111101'::uuid, 'b2222222-2222-4222-8222-222222222222'::uuid, '전 첫 만남 뒤 사적인 질문이 빨라지면 그 주제는 아직 편하지 않다고 바로 말하는 편이에요. 애매하게 넘기면 더 길어지더라고요.', false, 'schoolDepartment'::public.visibility_level),
      ('51111111-1111-4111-8111-111111111202'::uuid, '41111111-1111-4111-8111-111111111101'::uuid, 'c3333333-3333-4333-8333-333333333333'::uuid, '저는 질문 하나 선 넘었다 싶으면 대답 길게 안 하고 주제를 돌립니다. 그래도 반복되면 그 자리에서 선 긋는 게 제일 낫더라고요.', false, 'schoolDepartment'::public.visibility_level),
      ('51111111-1111-4111-8111-111111111203'::uuid, '41111111-1111-4111-8111-111111111102'::uuid, 'a1111111-1111-4111-8111-111111111111'::uuid, '집 얘기 바로 나오면 아직 그런 단계는 아니라고 짧게 말하는 편이 제일 깔끔했어요. 분위기 안 깨려다 더 불편해지는 경우가 많았습니다.', false, 'schoolDepartment'::public.visibility_level),
...
with seeded_notifications as (
  ...
  from (
    values
      ('81111111-1111-4111-8111-111111111501'::uuid, 'a1111111-1111-4111-8111-111111111111'::uuid, 'comment'::public.notification_type, '내 글에 새 댓글이 달렸습니다', '축제 후기 글에 새로운 댓글이 달렸습니다.', false, null::timestamptz, '/community?filter=hot&post=41111111-1111-4111-8111-111111111101', 'post', '41111111-1111-4111-8111-111111111101'::uuid, 'activity', 'instant', '{"postId":"41111111-1111-4111-8111-111111111101"}'::jsonb),
      ('81111111-1111-4111-8111-111111111502'::uuid, 'a1111111-1111-4111-8111-111111111111'::uuid, 'reply'::public.notification_type, '답글이 이어졌습니다', '내가 남긴 댓글에 답글이 달렸습니다.', false, null::timestamptz, '/community?filter=advice&post=41111111-1111-4111-8111-111111111161', 'comment', null::uuid, 'activity', 'instant', '{"postId":"41111111-1111-4111-8111-111111111161"}'::jsonb),
      ('81111111-1111-4111-8111-111111111503'::uuid, 'b2222222-2222-4222-8222-222222222222'::uuid, 'trade_match'::public.notification_type, '교환 매칭 후보가 생겼습니다', '원하는 강의를 가진 글이 새로 올라왔습니다.', false, null::timestamptz, '/trade?post=71111111-1111-4111-8111-111111111405&chat=1', 'trade', '71111111-1111-4111-8111-111111111405'::uuid, 'activity', 'instant', '{"tradePostId":"71111111-1111-4111-8111-111111111405"}'::jsonb),
      ('81111111-1111-4111-8111-111111111504'::uuid, 'c3333333-3333-4333-8333-333333333333'::uuid, 'trending_post'::public.notification_type, '내 글이 인기글로 올라왔습니다', '반응이 빠르게 붙어 커뮤니티 인기글에 노출되고 있습니다.', true, '2026-03-20T16:25:00+09:00'::timestamptz, '/community?filter=hot&post=41111111-1111-4111-8111-111111111103', 'post', '41111111-1111-4111-8111-111111111103'::uuid, 'activity', 'daily', '{"postId":"41111111-1111-4111-8111-111111111103"}'::jsonb),
      ('81111111-1111-4111-8111-111111111505'::uuid, 'a1111111-1111-4111-8111-111111111111'::uuid, 'lecture_reaction'::public.notification_type, '강의평 반응이 늘고 있습니다', '내가 남긴 강의평을 저장하고 다시 보는 사용자가 많아졌습니다.', true, '2026-03-20T13:15:00+09:00'::timestamptz, '/lectures/31111111-1111-4111-8111-111111111114', 'lecture', '31111111-1111-4111-8111-111111111114'::uuid, 'activity', 'instant', '{}'::jsonb),
      ('81111111-1111-4111-8111-111111111506'::uuid, 'e5555555-5555-4555-8555-555555555555'::uuid, 'admission_answer'::public.notification_type, '입시 질문에 답변이 달렸습니다', '질문에 대학생 답변이 달렸습니다. 상세에서 바로 확인해보세요.', false, null::timestamptz, '/school?tab=admission&post=41111111-1111-4111-8111-111111111111', 'post', '41111111-1111-4111-8111-111111111111'::uuid, 'activity', 'instant', '{}'::jsonb),
      ('81111111-1111-4111-8111-111111111507'::uuid, 'f6666666-6666-4666-8666-666666666666'::uuid, 'school_recommendation'::public.notification_type, '우리학교에서 많이 보는 글을 추천합니다', '같은 학교 학생들이 자주 열어보는 글을 모아봤습니다.', true, '2026-03-20T10:05:00+09:00'::timestamptz, '/school', 'system', null::uuid, 'recommendation', 'daily', '{"recommended": true}'::jsonb),
      ('81111111-1111-4111-8111-111111111508'::uuid, 'd4444444-4444-4444-8444-444444444444'::uuid, 'freshman_trending'::public.notification_type, '새내기존 질문이 활발합니다', '예비입학생들이 많이 보는 질문을 확인해보세요.', true, '2026-03-20T10:35:00+09:00'::timestamptz, '/school?tab=freshman', 'system', null::uuid, 'recommendation', 'daily', '{"recommended": true}'::jsonb),
      ('81111111-1111-4111-8111-111111111509'::uuid, 'a1111111-1111-4111-8111-111111111111'::uuid, 'verification_approved'::public.notification_type, '학교 메일 인증이 승인되었습니다', '이제 대학생 전용 기능을 사용할 수 있습니다.', false, null::timestamptz, '/profile', 'verification', null::uuid, 'system', 'instant', '{}'::jsonb),
      ('81111111-1111-4111-8111-111111111510'::uuid, 'a1111111-1111-4111-8111-111111111111'::uuid, 'announcement'::public.notification_type, '이번 주 운영 공지', '댓글 반응, 추천 알림, 인증 상태를 알림 탭에서 바로 확인할 수 있습니다.', true, '2026-03-20T13:15:00+09:00'::timestamptz, '/community', 'system', null::uuid, 'system', 'daily', '{}'::jsonb)
  ) as t(id, user_id, type, title, body, is_read, read_at, href, target_type, target_id, source_kind, delivery_mode, metadata)
)
...
values
  (
    ...
    '41111111-1111-4111-8111-111111111211'::uuid,
    (select id from public.users where email = 'yeji@ewha.ac.kr'),
    '저도 혼자 있는 시간이 더 편해진 쪽이었어요. 학교에서 혼밥하는 부담이 생각보다 빨리 사라지더라고요.',
    false,
    'anonymous'::public.visibility_level
  ),
  (
    ...
    '41111111-1111-4111-8111-111111111212'::uuid,
    (select id from public.users where email = 'hyobin@korea.ac.kr'),
    '등하교용이면 너무 잔잔한 곡보다 리듬 있는 인디 쪽이 오래 듣기 좋더라고요. 전 혁오나 실리카겔 자주 듣습니다.',
    false,
    'anonymous'::public.visibility_level
  ),
  (
    ...
    '41111111-1111-4111-8111-111111111215'::uuid,
    (select id from public.users where email = 'yonji@yonsei.ac.kr'),
    '공강 2시간이면 도서관 짧게 갔다가 카페 한 군데 들르는 루틴이 제일 무난했어요. 멀리 나가면 돌아오기 애매하더라고요.',
    false,
    'anonymous'::public.visibility_level
  ),
  (
    ...
    '41111111-1111-4111-8111-111111111218'::uuid,
    (select id from public.users where email = 'danbi@hufs.ac.kr'),
    '첫 모임은 카톡으로 역할, 마감, 회의 주기까지만 정해도 대부분 괜찮았습니다. 대면은 일정 안 맞을 때만 추가했어요.',
    false,
    'anonymous'::public.visibility_level
  )
on conflict (id) do update
